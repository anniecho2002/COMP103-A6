...
public class MazeSearch {
    ...
    public boolean exploreFromCell(MazeCell cell) {
        ...
        if(cell.isVisited()){ return false; }
            ...
            for(MazeCell c: cell){
                boolean placeholder = exploreFromCell(c);
                if(placeholder){ answer = true; }
            }
    ...
    public void exploreFromCellAll(MazeCell cell) {
        ...
        if(cell.isVisited() == false){
            cell.visit();
            if(cell != maze.getGoal()){ cell.draw(Color.yellow); }
            UI.sleep(delay);
            if(cell != maze.getGoal()){
                for(MazeCell c: cell){ exploreFromCellAll(c); }
            }
            cell.unvisit();
            if(cell != maze.getGoal()){ cell.draw(Color.white); }
            // ensures that all the cells are colored correctly; goal is green
        }
    }
